...
import Sponge.NonPipelined.SHAKE256N qualified as SHAKE256N
...
i64o64Core =
  PipeCtrl2Net $ \outputReady flushSig inputStream ->
    let inputWithFlush = bundle (inputStream, outputReady, flushSig)
        (outputStream, inputReady) = unbundle (SHAKE256N.sponge spongeFSM inputWithFlush)
  ...
  ( Synthesize
      { t_name = "dut",
        ...
        t_output =
          PortProduct
            ""
            [ PortProduct "SHAKE" [PortName "TDATA", PortName "TVALID", PortName "TLAST"],
              PortName "MSG_TREADY"
            ]
      }
  )
