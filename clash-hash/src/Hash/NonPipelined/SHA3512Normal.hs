...
module Hash.NonPipelined.SHA3512Normal
...
import Sponge.NonPipelined.SHA3512N qualified
...
{-# ANN
  topEntity
  ( Synthesize
      { t_name = "SHA3_512_NonPipelined_Normal",
        t_inputs =
          [ PortName "CLK",
            PortName "RST",
            PortName "EN",
...
topEntity clk rst en treadySig inputSig =
  withClockResetEnable clk rst en
    $ let (msgSig, flushSig) = unbundle inputSig
       in Sponge.NonPipelined.SHA3512N.sponge @System spongeFSM (bundle (msgSig, treadySig, flushSig))
