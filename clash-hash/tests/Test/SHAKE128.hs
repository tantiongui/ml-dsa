...
module Test.SHAKE128 (spec) where

import AXI4Stream (PipeCtrl2)
import Clash.Prelude (BitVector, System)
import Component.SHAKE128 qualified as SHAKE128
...
i64o64AsPipeCtrl = SHAKE128.i64o64Core
...
simulate flushPattern inputTiming =
  let outputBytes = 64
      ...
      outputWords = toWords64 (Crypton.shake128 outputBytes (BS.pack (P.concatMap bv64ToBytes inputValues)))
      firstOutputIdx =
        if P.null inputValues
          then case L.findIndex P.id flushPattern of
            Just i -> i P.+ 24
            Nothing -> P.error "Test.SHAKE128.simulate: empty input requires flush"
          else
            let lastBeatIdx =
                  case [i | (i, Just _) <- P.zip [0 ..] inputPattern] of
                    [] -> P.error "Test.SHAKE128.simulate: no input provided"
                    xs -> P.last xs
                beatCount = P.length inputValues
                permuteDelay = if beatCount P.== 21 then 48 else 24
             in lastBeatIdx P.+ permuteDelay
   in [Silent firstOutputIdx, Output outputWords]

genMessageBeats :: Gen [BitVector 64]
genMessageBeats = do
  beatCount <- chooseInt (1, 21)
...
spec = describe "SHAKE3-128" $ do
  ...
  it "matches expected output (21-beat input boundary case)" $ do
    let block = P.replicate 21 (toBV @64 ("ABCDEFGH" :: ByteString))
