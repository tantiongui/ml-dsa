...
i272o512AsPipe (outReady, inStream) =
  let (outStream, inReady) =
        unbundle (GX2.i272o512 clockGen resetGen enableGen (bundle (inStream, outReady)))
...
simulate inputTiming =
  let (inputPattern, inputValues) = expandInputTiming inputTiming
      inputBV =
        case inputValues of
          (v : _) -> v
          [] -> P.error "Test.GX2.simulate: no input provided"
      startSilence =
        case L.findIndex isJust inputPattern of
          Just i -> i
          Nothing -> P.error "Test.GX2.simulate: no input provided"
      inputBS = bvToBS 33 inputBV
      (rho, sigma) = gReference inputBS
      out0 = toBV @256 rho
      out1 = toBV @256 sigma
      base = [Silent 12, Output [out1 ++# out0]]
