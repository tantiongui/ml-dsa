...
simulate flushPattern inputTiming =
  let (inputPattern, inputValues) = expandInputTiming inputTiming
      digestWords = toDigestWords (Crypton.sha3 (BS.pack (P.concatMap bv64ToBytes inputValues)))
      firstOutputIdx =
        if P.null inputValues
          then case L.findIndex P.id flushPattern of
            Just i -> i P.+ 24
            Nothing -> P.error "Test.SHA3256.simulate: empty input requires flush"
          else
            let lastBeatIdx =
                  case [i | (i, Just _) <- P.zip [0 ..] inputPattern] of
                    [] -> P.error "Test.SHA3256.simulate: no input provided"
                    xs -> P.last xs
